...
const int thr_per_blk = ln_per_blk/ln_per_thr; //2^8
...
__global__ void full_blk_dive(int * data, int N, int phase, int first_step){
    ...
    int blk_per_sector = lns_per_sector/ln_per_blk;
    int blk_assigned_sector = blockIdx.x / blk_per_sector;
    int blk_spacing = blk_per_sector;
    int blk_sector_start = blk_assigned_sector * lns_per_sector;
    int blk_sector_end = (blk_assigned_sector+1) * lns_per_sector;
    int blk_sector_id = blockIdx.x % blk_per_sector;
    
    //transfer partition to shared mem
    __shared__ int blk_data[ln_per_blk];
    for (int i = 0; i < ln_per_thr; i++){
        int global_addr = blk_sector_start + blk_sector_id + blk_spacing * (i + threadIdx.x * ln_per_thr);
        int shared_addr = i + threadIdx.x * ln_per_thr;
        blk_data[shared_addr] = data[global_addr];
    }
    __syncthreads();
    ...
    for (int i = 0; i < ln_per_thr; i++){
        int global_addr = blk_sector_start + blk_sector_id + blk_spacing * (i + threadIdx.x * ln_per_thr);
        int shared_addr = i + threadIdx.x * ln_per_thr;
        data[global_addr] = blk_data[shared_addr];
    }
}
...
void blk_parallel_implementation(int * input, int * output, int N){
    ...
    for (int phase = 0; phase < round(log2(padded_N)); phase++){
